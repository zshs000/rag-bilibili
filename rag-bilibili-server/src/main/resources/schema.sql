...
CREATE TABLE IF NOT EXISTS `chunk` (
    ...
    `cid` BIGINT COMMENT 'B站分P CID',
    `page_number` INT COMMENT '视频分P序号',
    `start_time_ms` BIGINT COMMENT '分片起始时间（毫秒）',
    `end_time_ms` BIGINT COMMENT '分片结束时间（毫秒）',
    `subtitle_language` VARCHAR(32) COMMENT '字幕轨语言',
    `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY (`id`),
