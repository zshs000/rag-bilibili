...
| 4001 | 向量删除失败 |
| 4002 | 向量写入失败 |
| 4003 | RAG 服务暂不可用（未配置、被禁用或依赖初始化失败） |
...
RAG 不可用时返回业务错误 `4003`，且不会创建视频或分片数据。
...
- `4003`：RAG 配置不完整、被 `RAG_ENABLED=false` 禁用，或向量服务初始化失败；不会创建批次。
...
建立 SSE 前若 RAG 不可用，返回普通 `Result` 业务错误 `4003`，且不会保存本次用户消息。
